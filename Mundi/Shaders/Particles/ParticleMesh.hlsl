...
/**
 * GPU에 전달되는 메시 파티클 인스턴스 데이터.
 * C++의 FMeshParticleInstanceVertex 구조체와 1:1 대응합니다.
 */
struct FMeshParticleInstanceVertex
{
	// C++: FLinearColor (float4, 16 bytes)
	float4 Color;

	// C++: FMatrix (float4x4, 64 bytes)
	row_major float4x4 Transform;

	// C++: FVector4 (float4, 16 bytes)
	float4 Velocity;

	// C++: int32 SubUVParams[4] (int4, 16 bytes)
	int4 SubUVParams;

	// C++: float SubUVLerp (4 bytes)
	float SubUVLerp;

	// C++: float RelativeTime (4 bytes)
	float RelativeTime;
	
	// 명시적인 16바이트 정렬을 위한 8바이트 패딩 (SubUVLerp + RelativeTime = 8 bytes).
	// 총 구조체 크기를 16의 배수(128바이트)로 맞춥니다.
	float2 Pad;
};

// 파티클 데이터를 담는 구조화 버퍼 (VS에서만 사용)
StructuredBuffer<FMeshParticleInstanceVertex> ParticleBuffer : register(t10);
...
PS_INPUT mainVS(VS_INPUT input)
{
	...
	FMeshParticleInstanceVertex particle = ParticleBuffer[input.InstanceID];

	// 정점의 월드 위치 계산
	// 파티클 위치를 기준으로, 카메라의 Right/Up 벡터와 파티클 크기를 사용하여 오프셋 적용
	float4 worldPosition = mul(float4(input.Position, 1.0f), particle.Transform);
