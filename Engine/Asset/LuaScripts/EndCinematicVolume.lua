------------------------------------------------------------------
-- EndCinematicVolume.lua
--
-- 이 볼륨(Owner)에 플레이어가 들어오면 지정된 카메라로 뷰를
-- 1초간 블렌드하고, 나가면 다시 플레이어 뷰로 되돌림
-- GlobalTimeDilation을 0.1로 설정, 나가면 다시 되돌림
--
-- [사전 정의된 전역 변수]
...
local TimeDilationScale = 0.2
local EndDelayTime = 3.0
...
function BeginPlay()
    World = GetWorld()
    GetTimeManager():SetGlobalTimeDilation(1.0)
end
...
function OnActorBeginOverlap(overlappedActor, otherActor)
    ...
    if otherActor.Tag == CollisionTag.Player then
        ...
        CameraManager:SetViewTargetWithBlend(Owner, EndDelayTime * TimeDilationScale)
        GetTimeManager():SetGlobalTimeDilation(TimeDilationScale)
        
        Self:StartCoroutine("EndGame")
    end
end

function EndGame()
    coroutine.yield(EndDelayTime * TimeDilationScale)

    local World = GetWorld()
    if World then
        World:GetGameMode():EndGame()
    end
end
